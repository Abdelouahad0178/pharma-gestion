 ...
 * - Article: { produit, commandee:{...}, recu:{...}|null }
 * - Réception multi-lots avec MAJ stock_entries (PAS de stock classique)
 * - Impression responsive (mobile/desktop) avec cachet image/texte
 */
...
export default function Achats() {
  ...
  const injectStyles = useCallback(() => {
    ...
    style.textContent = `
      ...
      .section-title{
        margin:0 0 12px 0; font-weight:800; color:var(--text);
      ...
      .field,.select,.button{ font:inherit; border-radius:10px; }
      .field,.select{
        border:1px solid var(--border); padding:10px 12px; outline:none;
        background:#FFFFFF; color:var(--text);
        transition: box-shadow .15s, border-color .15s, background .15s;
      }
      .field::placeholder{ color:black; }
      .field:focus,.select:focus{
        border-color:var(--ring); box-shadow:0 0 0 4px rgba(165,180,252,.35);
        background:#FFFFFF;
      }

      /* ===== Boutons unifiés + hover cohérent ===== */
      .btn{
        padding:10px 14px; font-weight:700; border:1px solid transparent; border-radius:12px;
        cursor:pointer; transition: transform .12s ease, box-shadow .2s ease, filter .2s ease, background .2s ease, border-color .2s ease;
      ...
      .btn:focus-visible{
        outline:none; box-shadow:0 0 0 4px rgba(165,180,252,.45);
      }
      ...
      .btn-outline:hover{
        background:var(--outline-hover-grad);
        border-color:#BFDBFE;
      ...
      .btn-success{
        color:#064E3B; background:linear-gradient(135deg,#ECFDF5 0%, #DCFCE7 100%);
      ...
      .btn-success:hover{
        ...
        border-color:#34D399; filter:brightness(1.02);
      }

      .notice{
        border-radius:12px; padding:12px; font-weight:600; margin-bottom:12px;
        border:1px solid var(--border);
      }
      ...
      .table thead th{
        ...
        color:#E5E7EB; font-weight:800; text-transform:uppercase; font-size:12px; letter-spacing:.5px;
      ...
      .table tbody td{
        ...
        color:var(--text); font-weight:600; background:#fff;
      }
      ...
      .chip{
        ...
        background:#FDF2F8; color:#BE185D; display:inline-block; border:1px solid #FBCFE8; /* accent rose */
      }
      .qty{
      ...
      /* ====== Panneaux repliables (filtres & formulaire) ====== */
      .controls-bar{
        display:flex; align-items:center; gap:8px; justify-content:flex-start; flex-wrap:wrap;
        margin-bottom:10px;
      }
  ...
  /* ================= Collections locales ================= */
  const [articles, setArticles] = useState([]); // [{ produit, commandee:{...}, recu:null }]
  ...
  /* ====== Panneau repliable du formulaire ====== */
  ...
  /* ================= Saisie produit -> auto-suggest prix/fournisseur ================= */
  ...
  /* ================= MAJ stock_entries après réception ================= */
  const updateStockOnAdd = useCallback(
    async (payload) => {
      // Uniquement sur stock_entries : PAS de stock classique
  ...
  const handleSubmitReception = useCallback(async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "societe", societeId, "activities"), {
        ...
        userEmail: user.email,
        timestamp: Timestamp.now(),
        details: { achatId: receptionId, statut, action: "confirmation" },
      });
  ...
  const handleDeleteBon = useCallback(
    async (bon) => {
      ...
      try {
        ...
        // Si le bon a déjà généré des entrées de stock, on les supprime
...
<body><pre style="font-family:monospace">${JSON.stringify(
  ...
  const generatePrintHTML = useCallback(
    (bon, arts, total, cachetHtml, isMobileDevice = false) => {
...
<head>
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .document-container{background:#fff;max-width:100%;margin:0 auto;border-radius:0;position:relative;min-height:${isMobileDevice ? "calc(100vh - 10px)" : "calc(100vh - 20px)"};display:flex;flex-direction:column}
    .header-section{background:linear-gradient(135deg,#0B1220 0%,${primaryColor} 100%);padding:${isMobileDevice ? "15px 10px" : "20px 15px"};text-align:center;position:relative;overflow:hidden;flex-shrink:0}
    .company-title{color:#fff;font-size:${isMobileDevice ? "1.4em" : "1.8em"};font-weight:800;margin-bottom:${isMobileDevice ? "5px" : "8px"};text-shadow:2px 2px 4px rgba(0,0,0,.3);letter-spacing:${isMobileDevice ? ".5px" : "1px"}}
    .document-badge{background:rgba(255,255,255,.92);color:${primaryColor};padding:${isMobileDevice ? "4px 12px" : "6px 16px"};border-radius:20px;font-size:${isMobileDevice ? ".8em" : "1em"};font-weight:700;text-transform:uppercase;letter-spacing:${isMobileDevice ? ".5px" : "1px"};display:inline-block;border:1px solid rgba(255,255,255,.3)}
    ...
    .status-badge{display:inline-block;padding:${isMobileDevice ? "6px 12px" : "10px 20px"};border-radius:30px;font-weight:700;font-size:${isMobileDevice ? ".8em" : ".9em"};text-transform:uppercase;letter-spacing:1px}
    ...
    .articles-table th{padding:${isMobileDevice ? "8px 6px" : "12px 10px"};text-align:center;color:#fff;font-weight:700;font-size:${isMobileDevice ? ".8em" : ".9em"};text-transform:uppercase;letter-spacing:1px;border-right:1px solid rgba(255,255,255,.08)}
    ...
    .grand-total-section{margin:${isMobileDevice ? "25px" : "40px"} 0;padding:${isMobileDevice ? "20px 15px" : "30px"};background:linear-gradient(135deg,${primaryColor} 0%,${secondaryColor} 100%);border-radius:${isMobileDevice ? "10px" : "20px"};color:#fff;text-align:center;position:relative;overflow:hidden}
    .total-label{font-size:${isMobileDevice ? "1.1em" : "1.4em"};font-weight:700;margin-bottom:${isMobileDevice ? "8px" : "15px"};text-transform:uppercase;letter-spacing:${isMobileDevice ? "1px" : "2px"};opacity:.96}
    .total-amount{font-size:${isMobileDevice ? "2em" : "3em"};font-weight:900;text-shadow:2px 2px 4px rgba(0,0,0,.25);margin-bottom:${isMobileDevice ? "10px" : "20px"}}
    .signature-section{margin:${isMobileDevice ? "30px" : "40px"} 0;display:${isMobileDevice ? "block" : "flex"};justify-content:space-between;align-items:flex-end;gap:${isMobileDevice ? "20px" : "40px"}}
    .signature-box{ text-align:center; flex:1; max-width:${isMobileDevice ? "100%" : "200px"}; margin-bottom:${isMobileDevice ? "20px" : "0"}; }
    ...
    .signature-label{font-weight:800;color:#1E293B;font-size:${isMobileDevice ? ".8em" : ".9em"};text-transform:uppercase;letter-spacing:1px}
...
<body>
    ...
    <div class="content-wrapper">
      <div class="info-section">
        ...
        <div class="info-card">
          ...
          <div class="info-value">#${String(bon.id || "").substring(0, 8).toUpperCase()}</div>
        </div>
        <div class="info-card">
          <div class="info-label">💳 Statut Paiement</div>
          <div class="info-value"><span class="status-badge status-${(bon.statutPaiement || "paye").toLowerCase()}">${bon.statutPaiement || ""}</span></div>
        </div>
      </div>
      ...
      <div class="grand-total-section">
        <div class="total-label">💰 Montant Total ${bon.statutReception === "en_attente" ? "Commande" : "Réception"}</div>
  ...
  // Branche l’implémentation dans le ref
  useEffect(() => {
  ...
  function showMobileDownloadOption(htmlContent, titleDocument, numero) {
    ...
    modal.querySelector("#mp_newtab")?.addEventListener("click", () => {
  ...
  if (waiting) {
    return (
      <div className="achats-page" style={{ padding: 16 }}>
        <div className="card" style={{ background: "linear-gradient(135deg,#EEF2FF,#FFFFFF)" }}>
  ...
  return (
    <div className="achats-page" style={{ padding: 16, background: "var(--bg)" }}>
      ...
      <div className="page-header">
        ...
        <div className="page-sub">Commandes, réceptions, impression & traçabilité</div>
      </div>
      ...
      <div className="card">
        <div className="section-title" style={{ justifyContent: "space-between" }}>
          ...
          <div className="controls-bar">
            ...
            {articles.length > 0 && (
              <span className="filters-badge" title="Articles saisis">
        ...
        <div
          id="create-form-panel"
          className={`form-panel ${showCreateForm ? "form-shown" : "form-hidden"}`}
        ...
        >
          <div className="form-panel-inner">
            ...
            <div className="article-grid">
              <input
                className="field"
                placeholder="Produit (ou choisir)"
                value={produit}
              ...
              <input
                className="field"
                placeholder="Nouveau produit (si _new_)"
      ...
      <div className="card">
        <div className="section-title" style={{ justifyContent: "space-between" }}>
          ...
          <div className="controls-bar">
            ...
            {activeFiltersCount > 0 && (
              <span className="filters-badge" title="Filtres actifs">
        ...
        {/* Panneau repliable des filtres */}
        <div
          id="filters-panel"
          className={`filters-panel ${showFilters ? "filters-shown" : "filters-hidden"}`}
        ...
        >
          <div className="filters-panel-inner">
            <div className="form-grid" style={{ marginBottom: 8 }}>
              ...
              >
                <option value="">Tous statuts réception</option>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              {activeFiltersCount > 0 && (
                <div className="filters-badge">
                  {filteredAchats.length} résultat{filteredAchats.length > 1 ? "s" : ""}
                </div>
              )}
            </div>
          </div>
        ...
        {filteredAchats.length === 0 ? (
          <div style={{ color: "var(--muted)", marginTop: 8 }}>
            Aucun bon correspondant {activeFiltersCount ? "aux filtres." : "aux critères."}
          </div>
        ) : (
            ...
            <table className="table">
              ...
              <tbody>
                {filteredAchats.map((b) => (
                  <tr key={b.id}>
                    <td className="left">{b.fournisseur}</td>
